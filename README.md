...
| `/add_filter <ссылка>` | Добавить фильтр (или просто прислать URL) |
| `/list_filters` | Список фильтров с id |
| `/remove_filter <id>` | Удалить |
| `/pause_filter <id>` / `/resume_filter <id>` | Пауза / возобновить |
| `/status` | Сколько фильтров, последняя проверка, сколько лотов |
| `/set_interval <минуты>` | Частота проверки (не чаще 5 минут) |
| `/help` | Список команд |
